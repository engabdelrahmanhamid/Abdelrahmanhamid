import type { Metadata } from "next";
import localFont from "next/font/local";
import "./globals.css";
import NavBar from "@/components/NavBar";

const thmanyahSerifDisplay = localFont({
  src: [
    { path: "../public/fonts/thmanyahserifdisplay-Light.otf", weight: "300", style: "normal" },
    { path: "../public/fonts/thmanyahserifdisplay-Regular.otf", weight: "400", style: "normal" },
    { path: "../public/fonts/thmanyahserifdisplay-Medium.otf", weight: "500", style: "normal" },
    { path: "../public/fonts/thmanyahserifdisplay-Bold.otf", weight: "700", style: "normal" },
    { path: "../public/fonts/thmanyahserifdisplay-Black.otf", weight: "900", style: "normal" },
  ],
  variable: "--font-thmanyah-serif-display",
  display: "swap",
});

export const metadata: Metadata = {
  title: "Abdelrahman Hamid | Web Designer & Developer",
  description: "Web designer and developer specializing in business websites, e-commerce stores, UI/UX, landing pages, SEO and digital analytics.",
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en" className={`${thmanyahSerifDisplay.variable} h-full antialiased`}>
      <body className="min-h-screen w-full overflow-x-hidden bg-white text-black">
        <NavBar />
        {children}</body>
    </html>
  );
}
